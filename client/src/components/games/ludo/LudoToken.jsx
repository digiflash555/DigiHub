...
const LudoToken = ({
    ...
    return (
        <motion.div
            ...
            initial={{ scale: 0.8 }}
            animate={{ scale: 1 }}
            whileHover={isMovable ? { scale: 1.3, zIndex: 50 } : {}}
            whileTap={isMovable ? { scale: 0.9 } : {}}
            onClick={(e) => {
                e.stopPropagation();
                if (isMovable && onClick) onClick(token);
            }}
            className={`relative w-6 h-6 sm:w-8 sm:h-8 md:w-9 md:h-9 rounded-full flex items-center justify-center transition-all place-self-center ${
                isMovable ? 'ring-4 ring-white animate-bounce cursor-pointer' : 'pointer-events-none opacity-90'
            }`}
        >
