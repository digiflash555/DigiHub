...
import { 
    LayoutDashboard, Shield, Award, MessageSquare, 
    ChevronRight, Sparkles, ArrowLeft, ChevronDown, 
    Calendar, Trophy, Settings, Users, FileText, 
    DollarSign, Activity, Mail, Scan, CheckCircle
} from 'lucide-react';
...
const AdminHeader = ({ 
    ...
    const getAdminModules = () => {
        ...
        // Common modules for all admin roles
        const commonModules = [
            {
                category: 'Core Management',
                items: [
                    { title: 'Admin Dashboard', path: '/admin/dashboard', icon: LayoutDashboard, badge: 'Home' },
                    { title: 'Create Event', path: '/admin/events/create', icon: Calendar, badge: 'New' },
                    { title: 'Manage Winners', path: '/admin/winners', icon: Trophy },
            ...
            },
            {
                ...
                items: [
                    ...
                    { title: 'Registration Templates', path: '/admin/registration-templates', icon: FileText },
                ]
            },
            {
                category: 'Operations & Analytics',
                items: [
                    { title: 'Attendance Records', path: '/admin/attendance', icon: CheckCircle },
                    { title: 'Feedback Management', path: '/admin/feedback', icon: MessageSquare },
                    { title: 'Feedback Templates', path: '/admin/feedback-templates', icon: FileText },
                    { title: 'Scanner Mode', path: '/scanner', icon: Scan },
                ]
            },
            {
                category: 'Communication & Finance',
                items: [
                    { title: 'Email Management', path: '/admin/email/compose', icon: Mail },
                    { title: 'User Care & Support', path: '/admin/support', icon: MessageSquare },
                    { title: 'Expenses & Reimbursements', path: '/admin/expenses', icon: DollarSign },
                ]
            },
            {
                category: 'System',
        ...
        if (role === 'Admin' || role === 'Head of the Department') {
            return commonModules;
        }

        if (role === 'Faculty') {
            return [
                commonModules[0], // Core Management
                commonModules[3], // Operations & Analytics
                commonModules[4], // Communication & Finance
                commonModules[5], // System
            ];
        }

        if (role === 'Class Coordinator' || role === 'Program Coordinator') {
            return [
                commonModules[0], // Core Management
                commonModules[3], // Operations & Analytics
                commonModules[4], // Communication & Finance
        ...
        if (role === 'Association Member' || role === 'Association Coordinator') {
            return [
                commonModules[0], // Core Management
                commonModules[1], // People & Directory
                commonModules[2], // Forms & Certifications
                commonModules[3], // Operations & Analytics
            ];
        }
    ...
    const getModuleByPath = (pathname) => {
        for (const group of adminModules) {
            for (const item of group.items) {
                if (pathname === item.path || (item.path !== '/admin/dashboard' && item.path !== '/' && pathname.startsWith(item.path))) {
                    return item;
                }
            }
        }
        return { title: 'Admin Portal', path: '/admin/dashboard', icon: Shield };
    };

    const currentModule = getModuleByPath(location.pathname);
    const displayTitle = customTitle || currentModule.title;
    const IconComponent = CustomIcon || currentModule.icon || Shield;
        ...
        <div className="w-full mb-8 space-y-4">
                                ...
                                >
                                    <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800/80 pb-3">
                                        <div className="flex items-center gap-2">
                                            <Shield className="w-4 h-4 text-indigo-600 dark:text-indigo-400" />
                                            <span className="text-xs font-black uppercase tracking-wider text-slate-500 dark:text-slate-400">Admin Navigation</span>
                                        </div>
            ...
            {showHero && (subtitle || customTitle) && (
                <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 text-white p-6 sm:p-7 shadow-xl border border-indigo-500/20">
                    ...
                    <div className="absolute bottom-0 left-1/3 -mb-8 w-64 h-64 bg-violet-500/15 rounded-full blur-3xl pointer-events-none" />

                    <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div className="flex items-start gap-4">
                            <div className="p-3.5 bg-gradient-to-tr from-indigo-600 to-violet-600 rounded-2xl shadow-lg shadow-indigo-500/30 border border-indigo-400/30 text-white flex-shrink-0">
