            ...
            if (hasCertificate && regId) {
                ...
                try {
                    ...
                    await downloadCertificateAsPDF(
                        ...
                        config,
                        registrationId,
                        `Certificate_${eventData.title.replace(/\s+/g, '_')}.pdf`
                    );
                    // Close modal first, then show success so toast isn't lost on unmount
                    onClose();
                    setTimeout(() => {
                        toast.dismiss('cert-gen');
                        toast.success(
                            '🎉 Feedback submitted & Certificate downloaded successfully!',
                            { duration: 6000, style: { fontWeight: 700 } }
                        );
                    }, 150);
                } catch (certError) {
                    onClose();
                    setTimeout(() => {
                        ...
                        toast.error(
                            'Certificate download failed. You can download it from your dashboard later.',
                            { duration: 6000 }
                        );
                    }, 150);
                }
