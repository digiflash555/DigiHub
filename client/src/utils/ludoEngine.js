...
export const getValidTokenMoves = (playerTokens = [], diceValue) => {
    if (!diceValue || !Array.isArray(playerTokens)) return [];
...
export const findCapturedTokens = (movingToken, targetStepCount, allTokensMap = {}) => {
    if (!movingToken || targetStepCount < 1 || targetStepCount > 51 || !allTokensMap) return [];
    ...
    Object.keys(allTokensMap).forEach(pColor => {
        if (pColor !== movingToken.player && Array.isArray(allTokensMap[pColor])) {
...
export const getBestAiMove = (player, allTokensMap = {}, diceValue) => {
    const playerTokens = (allTokensMap && allTokensMap[player]) || [];
