...
import {
    ...
    Shield, BookOpen, GraduationCap, BadgeCheck, Trophy, RefreshCw
} from 'lucide-react';
...
const Profile = () => {
    ...
    const [isWinner, setIsWinner] = useState(null); // null = not checked, false = not a winner, object = winner data
    ...
    useEffect(() => {
        const checkWinnerStatus = async () => {
            if (user && (user.role === 'Participant' || user.role === 'Student')) {
                try {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // yearAndDept, section, registrationNumber, gender, passoutYear can only be changed by Admin
            if (user?.role === 'Admin') {
    ...
    const isStudent = !user?.role || user.role === 'Participant' || user.role === 'Student' || user.role === 'Association Member' || user.role === 'Volunteer';
    ...
    const roleColors = {
        'Admin': 'from-indigo-600 to-violet-700',
        'Association Member': 'from-blue-500 to-indigo-600',
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-0 pb-24">
            <StudentHeader
                title="My Profile"
                subtitle={`${user?.username} · ${user?.role || 'Student'} · Manage your account and settings`}
                icon={User}
                showHero={false}
            />
                ...
                <div className="relative p-6 md:p-10 pb-0">
                    ...
                    <div className="flex flex-col items-center justify-center text-center gap-6 mb-0">
                        ...
                        <div className="relative flex-shrink-0 mx-auto">
                            ...
                            {isWinner ? (
                                <label
                                    className="absolute -bottom-2 -right-2 w-10 h-10 bg-white dark:bg-[#20242B] text-slate-900 rounded-full flex items-center justify-center cursor-pointer hover:bg-amber-50 dark:hover:bg-amber-500/10 transition-all shadow-xl dark:text-amber-400 border-2 border-amber-400"
                                ...
                                <div
                                    className="absolute -bottom-2 -right-2 w-10 h-10 bg-white/30 dark:bg-black/30 text-white/60 rounded-full flex items-center justify-center cursor-not-allowed shadow-xl border-2 border-white/20 backdrop-blur-sm"
                                    title="Only top performers can upload a profile picture"
                                >
                        ...
                        <div className="flex flex-wrap justify-center gap-2 md:gap-3 w-full">
                            ...
                            {isStudent && isWinner && (
                                <Link
                                    to={`/winners#winner-${isWinner._id || ''}`}
                                    className="px-3 md:px-4 py-2 bg-amber-500/20 rounded-2xl border border-amber-400/30 backdrop-blur-sm flex items-center gap-2 cursor-pointer hover:bg-amber-500/30 transition-all group"
                                    title="View your achievements on the Wall of Winners"
                                >
                                    <Trophy className="w-3.5 h-3.5 text-amber-300 group-hover:scale-110 transition-transform" />
                                    <p className="text-xs md:text-sm font-black text-amber-300">Top Performer</p>
                                </Link>
                            )}
                        </div>
                    </div>
            ...
            <AnimatePresence mode="wait">
                                ...
                                <>
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="grid md:grid-cols-3 gap-6">
                                            <div className="space-y-1.5">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-[10px] font-bold bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-400 px-2 py-0.5 rounded-full">Admin-only</span>
                                                </div>
                                                <div className="relative">
                                                    <input type="text" name="registrationNumber" className="input-premium pl-12 disabled:opacity-50 disabled:cursor-not-allowed" placeholder="e.g. 21CS101" value={formData.registrationNumber} onChange={handleChange} disabled />
                                                </div>
                                            </div>
                                            <div className="space-y-1.5">
                                                <div className="flex items-center gap-2">
                                                ...
                                                <select name="gender" className="input-premium disabled:opacity-50 disabled:cursor-not-allowed" value={formData.gender} onChange={handleChange} disabled>
                                        ...
                                        <div className="space-y-1.5">
                                            ...
                                            <select name="yearAndDept" className="input-premium disabled:opacity-50 disabled:cursor-not-allowed" value={formData.yearAndDept} onChange={handleChange} disabled>
                                                {filteredYearDeptOptions.map(opt => (
                                                    <option key={opt} value={opt}>{opt}</option>
                                                ))}
                                            </select>
                                            <p className="text-xs text-slate-400 pl-1">Updated by Admin only. Contact your administrator to change your year.</p>
                                        </div>
                                        <div className="space-y-1.5">
                                            <div className="flex items-center gap-2">
                ...
                {activeTab === 'Security' && (
                    ...
                    >
                        ...
                        {/* CLEAR CACHE SECTION */}
                        <div className="p-10 border-t border-slate-100 dark:border-slate-800 space-y-6 max-w-lg">
                            <div className="flex items-center gap-3">
                                <div className="w-8 h-8 bg-amber-100 dark:bg-amber-500/15 rounded-xl flex items-center justify-center">
                                    <RefreshCw className="w-4 h-4 text-amber-500" />
                                </div>
                                <div>
                                    <h2 className="text-lg font-black text-slate-900 dark:text-white">Clear Browser Cache</h2>
                                    <p className="text-xs text-slate-400 font-medium">Clear local storage and refresh to fix display issues.</p>
                                </div>
                            </div>
                            <button
                                type="button"
                                onClick={handleClearCache}
                                className="w-full py-4 bg-amber-500/10 text-amber-600 dark:text-amber-500 border border-amber-200 dark:border-amber-500/30 rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-amber-500 hover:text-white transition-all flex items-center justify-center gap-3"
                            >
