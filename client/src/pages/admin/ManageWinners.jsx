    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8 pb-20">
            <AdminHeader
                ...
                subtitle="Recognize and manage winner announcements for completed events."
                icon={Trophy}
            />
                                ...
                                <motion.button
                                    ...
                                    className={`w-full p-4 rounded-xl text-left transition-all group ${
                                        selectedEvent?._id === event._id
                                            ...
                                            : 'bg-slate-50 dark:bg-[#20242B] hover:bg-slate-100 dark:hover:bg-[#2a2e36] text-slate-900 dark:text-slate-100 border border-slate-200 dark:border-slate-700 hover:border-indigo-200 dark:hover:border-indigo-500/30'
                                    }`}
                                >
                    ...
                    {selectedEvent ? (
                        <>
                            ...
                            <motion.div 
                                ...
                                className="bg-gradient-to-br from-indigo-50 to-violet-50 dark:from-indigo-500/10 dark:to-violet-500/10 p-6 rounded-2xl shadow-sm border border-indigo-100 dark:border-indigo-500/20"
                            >
                            ...
                            <button
                                ...
                                className="group w-full p-4 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white font-bold rounded-xl flex items-center justify-center gap-2 transition-all shadow-lg shadow-indigo-500/25 hover:shadow-xl hover:shadow-indigo-500/30"
                            >
                            ...
                            <div className="bg-white dark:bg-[#1A1D24] p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 space-y-4">
                                    ...
                                    <button
                                        ...
                                        className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1 transition-colors border ${
                                            selectedEvent.winnerPhotoUploadLocked 
                                                ? 'bg-red-50 text-red-600 dark:bg-red-500/10 dark:text-red-400 border-red-100 dark:border-red-500/20' 
                                ...
                                {winners.length > 0 ? (
                                    <div className="space-y-3">
                                        {winners.map((winner, idx) => (
                                            <motion.div
                                                ...
                                                animate={{ opacity: 1, y: 0 }}
                                                transition={{ delay: idx * 0.05 }}
                                                className="group p-4 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-500/10 dark:to-orange-500/10 rounded-xl border border-amber-100 dark:border-amber-500/20 flex items-center justify-between hover:shadow-md transition-shadow"
                                            >
                                                <div className="flex items-center gap-4 flex-1">
                                                    <div className="w-16 h-16 rounded-xl overflow-hidden bg-slate-100 flex-shrink-0 relative border-2 border-amber-200 group-hover:scale-105 transition-transform">
                                                        <img 
                                                            src={getImageUrl(winner.profilePhoto || winner.participant?.profileImage || 'default-profile.png')} 
                                                            className="w-full h-full object-cover" 
                                                            alt="Winner" 
                                                        />
                                                        <div className="absolute -bottom-1 -right-1 w-6 h-6 bg-gradient-to-br from-yellow-400 to-orange-500 rounded-full flex items-center justify-center text-white font-black text-[10px] border-2 border-white shadow-lg">
                                                ...
                                                <div className="flex flex-col items-end gap-2 flex-shrink-0">
                                                    <div className="flex items-center gap-1">
                                                        <label className="group p-1.5 text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 rounded-lg transition-colors cursor-pointer" title="Replace Photo">
                                                        ...
                                                        {winner.profilePhoto && (
                                                            <button onClick={() => handleAdminPhotoDelete(winner._id)} className="group p-1.5 text-orange-500 hover:bg-orange-50 dark:hover:bg-orange-500/10 rounded-lg transition-colors" title="Remove Uploaded Photo">
                                                                <XCircle className="w-4 h-4 group-hover:scale-110 transition-transform" />
                                                            </button>
                                                        )}
                                                        <button
                                                            onClick={() => handleSingleLockToggle(winner._id, winner.photoUploadLocked)}
                                                            className={`group p-1.5 rounded-lg transition-colors ${winner.photoUploadLocked ? 'text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10' : 'text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'}`}
                                                            title={winner.photoUploadLocked ? "Unlock Photo Upload" : "Lock Photo Upload"}
                                                        >
                                                            {winner.photoUploadLocked ? <Lock className="w-4 h-4 group-hover:scale-110 transition-transform" /> : <Unlock className="w-4 h-4 group-hover:scale-110 transition-transform" />}
                                                        </button>
                                                        <button
                                                            onClick={() => handleDeleteWinner(winner._id)}
                                                            className="group p-1.5 text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg transition-colors ml-2"
                                                            title="Remove Winner"
                                                        >
                                    ...
                                    <div className="text-center py-8 space-y-3">
                                        <Trophy className="w-12 h-12 text-slate-200 mx-auto" />
                                        <p className="text-slate-500 dark:text-slate-400">No winners added yet</p>
                                    </div>
                                )}
                            </div>
                        </>
