...
const FeedbackManagement = () => {
    ...
    const downloadAnalyticalReport = async () => {
        ...
        try {
            ...
            doc.text('Feedback Analysis Report', 105, headerHeight, { align: 'center' });
            ...
            if (summary) {
                ...
                for (let qi = 0; qi < summaryEntries.length; qi++) {
                    ...
                    if (info.type === 'counts') {
                        ...
                        // Show up to 5 sample responses (no sentiment labels)
                        info.answers.slice(0, 5).forEach(({ text }) => {
                            if (currentY > 265) {
                                doc.addPage();
                                currentY = 15;
                            }
                            ...
                            const truncated = String(text).length > 100 ? String(text).substring(0, 100) + '...' : String(text);
    ...
    return (
        <div className="space-y-8 pb-20">
            <AdminTopBar 
                ...
                subtitle="Analyze responses, compute average ratings, and download detailed analytical reports for event feedback."
                icon={MessageSquare}
                ...
                actions={
                    selectedEvent && (
                        <div className="flex gap-3">
                            <Link
                                to="/admin/settings"
                                className="bg-white/10 text-white font-bold px-4 py-2.5 rounded-xl border border-white/10 flex items-center gap-2 hover:bg-white/20 transition-all shadow-sm"
                            ...
                            <button
                                ...
                                className="bg-white/10 text-white font-bold px-5 py-2.5 rounded-xl border border-white/10 flex items-center gap-2 hover:bg-white/20 transition-all shadow-sm disabled:opacity-50"
                            >
                                {isLoading ? (
                                        ...
                                        <div key={label} className="space-y-3 border-b border-gray-100 pb-8 last:border-0 last:pb-0">
                                            ...
                                            {info.type === 'text' && (
                                                <div className="space-y-2">
                                                    {(expandedTextFields[label] ? info.answers : info.answers.slice(0, 4)).map(({ text }, i) => (
                                                        <div key={i} className="flex gap-3 items-start">
                                                            <span className="text-sm text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-black/20 px-3 py-1.5 rounded-lg flex-1 italic w-full">
                                                                "{text}"
                                                            </span>
                                                        </div>
                                                    ))}
