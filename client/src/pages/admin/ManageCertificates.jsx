    ...
    return (
        <div className="space-y-8 pb-20">
            <AdminTopBar 
                ...
                subtitle="Design dynamic certificates with bold/italic formatting, dynamic placement, and bulk-send certificates to all participants." 
                icon={Award}
                    ...
                    selectedEventId && (
                        <div className="flex flex-wrap gap-3">
                            <button
                                ...
                                className="flex items-center gap-2 px-5 py-2.5 bg-white/10 border border-white/20 text-white font-bold rounded-xl hover:bg-white/20 transition-all disabled:opacity-50 shadow-sm"
                            >
                                {isPreviewing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Eye className="w-4 h-4" />}
                                Preview PDF
                            </button>

                            <div className="flex items-center border border-white/20 rounded-xl overflow-hidden shadow-sm">
                                <button
                                    onClick={handleBulkSend}
                                    disabled={isBulkSending || !config.template || !config.fields?.length}
                                    className="px-4 py-2.5 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-100 font-bold transition-all flex items-center gap-2 border-r border-white/20 disabled:opacity-50 text-sm"
                                >
                                ...
                                <select value={sendTarget} onChange={e => setSendTarget(e.target.value)} disabled={isBulkSending || isDownloadingPDF || isDownloadingPNG}
                                    className="bg-emerald-500/10 text-emerald-100 font-bold text-sm outline-none cursor-pointer px-3 py-2.5 border-r border-white/20 hover:bg-emerald-500/20 transition-all appearance-none">
                                    <option className="text-gray-900" value="both">All</option>
                                    <option className="text-gray-900" value="participants">Participants</option>
                                    <option className="text-gray-900" value="volunteers">Volunteers</option>
                                </select>
                                <button
                                    onClick={() => handleBulkDownload('pdf')}
                                    disabled={isDownloadingPDF || isDownloadingPNG || !config.template || !config.fields?.length}
                                    className="px-4 py-2.5 bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-100 font-bold transition-all flex items-center gap-2 border-r border-white/20 disabled:opacity-50 text-sm"
                                ...
                                <button
                                    ...
                                    className="px-4 py-2.5 bg-fuchsia-500/20 hover:bg-fuchsia-500/30 text-fuchsia-100 font-bold transition-all flex items-center gap-2 border-r border-white/20 disabled:opacity-50 text-sm"
                                ...
                                <button
                                    ...
                                    className="px-4 py-2.5 bg-blue-500/20 hover:bg-blue-500/30 text-blue-100 font-bold transition-all flex items-center gap-2 disabled:opacity-50 text-sm"
                            ...
                            <button onClick={saveConfig} disabled={isSaving} className="btn-premium flex items-center gap-2 px-5 py-2.5 text-sm shadow-lg">
