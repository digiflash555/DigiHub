...
const AdminDashboard = () => {
    ...
    const cards = [
        ...
        { title: 'Upcoming', val: stats.upcomingEvents || allEvents.filter(e => e.status === 'Upcoming').length, icon: <Calendar className="w-6 h-6" />, color: 'bg-violet-600', label: 'Future Events' },
        { title: 'Ongoing', val: stats.ongoingEvents || allEvents.filter(e => e.status === 'Ongoing').length, icon: <TrendingUp className="w-6 h-6" />, color: 'bg-amber-600', label: 'Currently Live' },
    ];
                ...
                <div className="space-y-8">
                    ...
                    <div className="grid gap-4">
                        {[
                            ...
                            { title: 'User Care', icon: <MessageSquare />, color: 'bg-indigo-100 text-indigo-600', link: '/admin/support' },
                            { title: 'System Settings', icon: <Shield />, color: 'bg-slate-100 text-slate-600', link: '/admin/settings' },
                            { title: 'Email Management', icon: <MessageSquare />, color: 'bg-indigo-100 text-indigo-600', link: '/admin/email/compose' },
                        ].map((action, i) => (
