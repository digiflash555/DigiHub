...
import {
    ...
    CheckCircle, History, RefreshCcw, Edit2, Trash2, X, Save, Download, Search, Award, Calendar
} from 'lucide-react';
...
const ManageAssociationMembers = () => {
    ...
    const [activeTab, setActiveTab] = useState('Present'); // 'Present' or 'Alumni'
    ...
    const fetchMembers = async () => {
        try {
            ...
            let fetchedMembers = res.data.filter(u => u.role === 'Association Member' || (u.role === 'Participant' && u.membershipStatus === 'Alumni'));
    ...
    const handleDownloadPDF = (filterType = 'All') => {
        ...
        if (filterType === 'Present') {
            ...
            membersToExport = members.filter(m => m.membershipStatus === 'Alumni');
        }

        if (membersToExport.length === 0) {
            toast.error(`No ${filterType === 'Alumni' ? 'Alumni' : filterType} members found to download.`);
            return;
        }
        ...
        if (filterType === 'Alumni') title = 'ALUMNI ASSOCIATION MEMBERS';
        ...
        const pastCount = membersToExport.filter(m => m.membershipStatus === 'Alumni').length;
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(9);
        ...
        const tableData = membersToExport.map((m, idx) => [
            ...
            m.membershipStatus === 'Alumni' ? 'Alumni' : (m.membershipStatus || '-')
        ]);
    ...
    const toggleMemberStatus = async (memberId, currentStatus) => {
        if (currentStatus === 'Alumni') {
            toast.error('Cannot restore Alumni from this button. Please edit their profile.');
            return;
        }
        
        const confirmed = await confirm('Are you sure you want to move this member to Alumni?\n\nThey will become a normal Student and lose all Association Member permissions.');
        if (!confirmed) return;
        
        try {
            await axios.put(`/api/auth/member-status/${memberId}`, { membershipStatus: 'Alumni' });
    ...
    return (
        <div className="space-y-8 pb-40">
            <AdminTopBar 
                ...
                subtitle="Manage student association member accounts and alumni directory." 
                icon={Users}
                        ...
                        <form onSubmit={handleSubmit} className="space-y-8">
                            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    {editingId && form.membershipStatus === 'Alumni' ? (
                                        <div className="input-premium bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 font-black flex items-center gap-2">
                                            <Award className="w-4 h-4" /> Alumni (use Delete/Re-register to restore)
                                        </div>
                                    ) : (
                                    <select
                                        className="input-premium"
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        <div className="flex flex-wrap items-center gap-2 md:gap-4">
                            ...
                            <div className="flex bg-slate-900 text-white rounded-xl overflow-hidden text-xs font-black shadow-sm">
                                ...
                                <button
                                    onClick={() => handleDownloadPDF('Alumni')}
                                    disabled={members.length === 0}
                                    className="px-4 py-2 hover:bg-slate-700 transition-all border-r border-slate-700 flex items-center gap-1.5 disabled:opacity-50"
                                >
                            ...
                            <div className="flex bg-slate-50 dark:bg-[#1a1d24] p-1.5 rounded-2xl gap-2">
                                ...
                                <button
                                    ...
                                    className={`px-4 md:px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'Alumni' ? 'bg-white shadow-sm text-amber-600' : 'text-slate-400 hover:text-slate-600'} dark:text-white`}
                                >
                                    Alumni
                                </button>
                            </div>
                                        ...
                                        <td className="px-6 md:px-10 py-4 md:py-6">
                                            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${
                                                member.membershipStatus === 'Present' 
                                                    ? 'bg-emerald-50 text-emerald-600 dark:text-emerald-400 border-emerald-100' 
                                                    : member.membershipStatus === 'Alumni'
                                                    ? 'bg-amber-50 text-amber-600 dark:text-amber-400 border-amber-100'
