...
const ComposeEmail = () => {
    ...
    useEffect(() => {
        ...
        axios.get('/api/email/class-groups').then(r => setClassGroups(r.data)).catch(() => {});
    }, []);
    ...
    const totalRecipients = mode === 'event'
        ...
        : mode === 'generic'
            ? specificEmails.length + activeGroupLabels.reduce((s, g) => s + (genericGroupCounts[g] || 0), 0)
            : specificEmails.length + activeGroupLabels.reduce((s, g) => {
                const cg = classGroups.find(c => c.name === g);
                return s + (cg ? cg.count : 0);
            }, 0);
    ...
    return (
        <div className="space-y-6 pb-6">
            ...
            <div className="flex gap-0 p-1 bg-slate-100 dark:bg-slate-800 rounded-2xl w-fit flex-wrap">
                ...
                <button onClick={() => { setMode('class'); setSelectedGroups({}); setSelectedEventId(''); setEventGroups(null); }}
                    className={`flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-black transition-all ${
                        mode === 'class'
                            ? 'bg-white dark:bg-[#20242B] text-indigo-600 shadow-sm'
                            : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'
            ...
            {/* ── Class Group Picker (mode=class) ── */}
            {mode === 'class' && (
                <div className="space-y-3 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/30">
                    <label className="text-xs font-black text-slate-600 dark:text-slate-300 uppercase tracking-widest flex items-center gap-2">
                        <Users className="w-4 h-4 text-indigo-500" /> Select Classes
                    </label>
                    <div className="flex flex-wrap gap-2">
                        {classGroups.map(c => (
                            <GroupChip
                                key={c.name}
                                label={c.name.replace('Class: ', '')}
                                count={c.count}
                                selected={!!selectedGroups[c.name]}
                                onClick={() => toggleGroup(c.name)}
                                colorClass={'bg-emerald-100 text-emerald-700 border-emerald-300 dark:bg-emerald-900/30 dark:text-emerald-300 dark:border-emerald-700'}
                            />
                        ))}
                        {classGroups.length === 0 && (
                            <p className="text-xs text-slate-400 font-medium">No class groups found.</p>
                        )}
                    </div>
                </div>
            )}
