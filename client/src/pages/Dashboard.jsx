                                        ...
                                        <div key={ev._id} className="p-6 bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/20 rounded-2xl hover:bg-indigo-100 transition-colors group">
                                            <div className="flex items-center gap-6 cursor-pointer" onClick={() => goToEvent(ev._id)}>
                                                ...
                                                <div className="flex flex-col items-end gap-2 shrink-0">
                                                    <span className={
                                                        (ev.status === 'Upcoming' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
                                                        ev.status === 'Ongoing' ? 'bg-amber-50 text-amber-700 border-amber-100' :
...
const Dashboard = () => {
    ...
    const [winnerStatuses, setWinnerStatuses] = useState([]);
    const [isLoading, setIsLoading] = useState(true);
        ...
        const fetchData = async () => {
            try {
                const requests = [
                    ...
                    axios.get(`/api/winners/my-status`)
                ];
                ...
                setWinnerStatuses(results[2].data);
                if (user?.role === 'Association Member' && results[3]) {
                    setInchargeEvents(results[3].data);
    ...
    const handlePhotoUpload = async (winnerId, e) => {
        ...
        try {
            ...
            setWinnerStatuses(prev => prev.map(w => w._id === winnerId ? { ...w, profilePhoto: res.data.profilePhoto } : w));
        } catch (err) {
    ...
    const downloadQRWithTemplate = (reg) => {
        ...
        logoImage.onload = () => {
            ...
            const wrapText = (context, text, maxWidth) => {
                ...
                // Break any lines that are still too wide (e.g., long words without spaces)
                const finalLines = [];
                lines.forEach(line => {
            ...
            if (titleLines.length > 2) {
                titleFontSize = 42;
                ctx.font = `900 ${titleFontSize}px Inter, sans-serif`;
                titleLines = wrapText(ctx, title, 680);
            }
            ...
            if (titleLines.length > 4) {
                ...
                titleLines[3] = titleLines[3].slice(0, -3) + '...';
            }
            
            const titleLineHeight = titleFontSize * 1.2;
            const titleStartY = 330 - ((titleLines.length - 1) * titleLineHeight) / 2;
            titleLines.forEach((line, index) => {
                ctx.fillText(line, 400, titleStartY + (index * titleLineHeight));
            });
            ...
            const venueText = reg.event.venue || '';
            let venueLines = wrapText(ctx, venueText, 280);
            
            if (venueLines.length > 2) {
                venueFontSize = 22;
                ctx.font = `bold ${venueFontSize}px Inter, sans-serif`;
                venueLines = wrapText(ctx, venueText, 280);
            }
            if (venueLines.length > 3) {
                venueFontSize = 14;
                ctx.font = `bold ${venueFontSize}px Inter, sans-serif`;
                venueLines = wrapText(ctx, venueText, 280);
            }
            if (venueLines.length > 4) {
                venueLines = venueLines.slice(0, 4);
                venueLines[3] = venueLines[3].slice(0, -3) + '...';
            }
            
            const venueLineHeight = venueFontSize * 1.2;
            const venueStartY = 495 - ((venueLines.length - 1) * venueLineHeight) / 2;
            ...
            qrImage.crossOrigin = 'anonymous'; // Ensure CORS is handled if qrCode becomes a URL
            qrImage.onload = () => {                // QR Code container with gradient background and subtle shadow
                ...
                const safeTitle = (reg.event?.title || 'Event').replace(/[^a-zA-Z0-9]/g, '_');
                link.download = `EventPass-${safeTitle}.png`;
                link.click();
            };
            qrImage.onerror = () => {
                toast.error('Failed to load QR code for the pass.');
            };
            qrImage.src = reg.qrCode;
        };
        logoImage.onerror = () => {
            toast.error('Failed to load logo for the pass.');
        };
        ...
        <div className={`space-y-12 pb-40 relative ${isBirthday ? 'birthday-theme' : ''}`}>
            ...
            <div className="grid lg:grid-cols-3 gap-10">
                ...
                <div className="lg:col-span-2 space-y-12">
                                    ...
                                    return (
                                        <a key={ev._id} href={`/events/${ev._id}`} className="flex flex-col gap-6 p-6 bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/20 rounded-[2.5rem] hover:bg-indigo-100 transition-colors">
                                            <div className="flex items-center gap-6 w-full">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <h3 className="font-extrabold text-slate-900 dark:text-white text-lg break-words">{ev.title}</h3>
                                                    <div className="flex flex-wrap gap-4 mt-1 text-sm text-slate-500 dark:text-slate-400 font-bold">
                                                        <span className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5" />{new Date(ev.eventDate).toLocaleDateString()}</span>
                                                        <span className="flex items-center gap-1"><MapPin className="w-3.5 h-3.5" />{ev.venue}</span>
                                                    </div>
                                                ...
                                                <div className="flex flex-col items-end gap-2 shrink-0">
                                                    <span className={`px-3 py-1.5 rounded-full text-xs font-black border ${ev.status === 'Upcoming' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : ev.status === 'Ongoing' ? 'bg-amber-50 text-amber-700 border-amber-100' : ev.status === 'Completed' ? 'bg-slate-100 text-slate-600 dark:text-slate-300 border-slate-200' : 'bg-red-50 text-red-700 border-red-100' }`}>{ev.status}</span>
                                                    <span className="px-3 py-1 bg-indigo-100 text-indigo-700 dark:text-indigo-300 rounded-full text-[10px] font-black uppercase tracking-wider">
                                                        {isFC ? 'Faculty Coord.' : 'Student Coord.'}
                                                    </span>
                                                </div>
                    ...
                    {/* My Winnings Section */}
                    {winnerStatuses.length > 0 && (
                        <div className="space-y-6">
                            <div className="flex items-center justify-between">
                                <h2 className="text-2xl font-black text-slate-900 dark:text-white">🏆 My Winnings</h2>
                                <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Excellence</span>
                            </div>
                            <div className="grid gap-6 md:grid-cols-2">
                                {winnerStatuses.map(winner => (
                                    <div key={winner._id} className="bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-500/10 dark:to-orange-500/10 p-6 rounded-[2.5rem] border border-amber-100 dark:border-amber-500/20 shadow-sm flex flex-col gap-4">
                                        <div className="flex items-start justify-between">
                                            <div>
                                                <h3 className="font-black text-xl text-slate-900 dark:text-white">{winner.event?.title}</h3>
                                                <p className="text-amber-600 dark:text-amber-400 font-bold">{winner.position} • {winner.prize}</p>
                                            </div>
                                            <div className="w-16 h-16 rounded-2xl overflow-hidden bg-white shadow-sm shrink-0 border-2 border-white">
                                                <img 
                                                    src={getImageUrl(winner.profilePhoto || user.profileImage || 'default-profile.png')} 
                                                    alt="Winner" 
                                        ...
                                        <div className="mt-2">
                                            {winner.photoUploadLocked || (winner.event && winner.event.winnerPhotoUploadLocked) ? (
                                                <div className="w-full py-3 px-4 bg-white/50 dark:bg-black/20 rounded-xl text-sm font-bold text-slate-500 flex items-center justify-center gap-2">
                                                    <XCircle className="w-4 h-4" /> Upload locked by Admin
                                                </div>
                                            ) : (
                                                <label className="w-full py-3 px-4 bg-amber-500 hover:bg-amber-600 text-white rounded-xl text-sm font-black flex items-center justify-center gap-2 cursor-pointer transition-colors shadow-sm">
                                                    <Upload className="w-4 h-4" /> 
                                                    {winner.profilePhoto ? 'Replace Photo' : 'Upload Profile Photo'}
                                                    <input 
                                                        type="file" 
                                                        accept="image/jpeg, image/png, image/jpg" 
                                                        className="hidden" 
                                                        onChange={(e) => handlePhotoUpload(winner._id, e)} 
                                                    />
                                                </label>
                                            )}
                                        </div>
                                    ...
                                    >
                                        <div className="flex gap-8 items-center">
                                            ...
                                            <div className="space-y-3 flex-1 min-w-0">
                                                <h3 className="text-2xl font-black text-slate-900 dark:text-white leading-tight break-words">{reg.event?.title || 'Unknown Event'}</h3>
                                                ...
                                                <div className="flex gap-2">
                                                    {reg.attendanceStatus ? (
                                                        ...
                                                        <span className="bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-2 border border-red-100 dark:border-red-500/20">
                                                            <XCircle className="w-3.5 h-3.5" /> Absent
                                                        </span>
                                                    ) : reg.event?.status === 'Ongoing' ? (
                                                        <span className="bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-2 border border-blue-100 dark:border-blue-500/20">
                ...
                {selectedReg && (
                    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md" onClick={() => setSelectedReg(null)}>
                        <motion.div
                            ...
                            className="bg-white dark:bg-[#20242B] p-8 rounded-[3rem] max-w-sm w-full text-center space-y-6 shadow-2xl relative dark:text-white max-h-[92vh] overflow-y-auto"
                            onClick={e => e.stopPropagation()}
                        >
                            {/* Close button */}
                            <button
                                onClick={() => setSelectedReg(null)}
                                className="absolute top-5 right-5 w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors"
                            >
                                <X className="w-4 h-4" />
                            </button>
                            ...
                            <div className="space-y-2 text-center pt-2">
                                <h3
                                    title={selectedReg.event?.title}
                                    className="font-black text-slate-900 dark:text-white leading-tight line-clamp-3"
                                    style={{ fontSize: (() => {
                                        const len = (selectedReg.event?.title || '').length;
                                        if (len > 80) return '1rem';
                                        if (len > 50) return '1.15rem';
                                        if (len > 30) return '1.35rem';
                                        return '1.6rem';
                                    })() }}
                                >
                                    {selectedReg.event?.title || 'Event Pass'}
                                </h3>
                                <p className="text-xs text-slate-400 font-bold uppercase tracking-widest line-clamp-1">{selectedReg.event?.venue}</p>
                            </div>
                            <div className="bg-white dark:bg-[#20242B] p-6 rounded-[2.5rem] border-4 border-slate-50 inline-block shadow-lg relative dark:text-white">
                                <img src={selectedReg.qrCode} alt="Registration QR" className="w-56 h-56 sm:w-64 sm:h-64" />
                                <div className="absolute inset-0 bg-primary-500/5 pointer-events-none rounded-[2.3rem]" />
                            </div>
                            <div className="space-y-1">
                                <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Digital Ticket ID</p>
                                <p className="text-base font-mono font-black text-slate-900 dark:text-white break-all">{selectedReg.registrationId}</p>
                            </div>
                            <button
                                onClick={() => downloadQRWithTemplate(selectedReg)}
                                className="w-full py-4 bg-primary-600 text-white font-black rounded-[2rem] flex items-center justify-center gap-3 hover:bg-primary-500 transition-all shadow-xl shadow-primary-200"
                            >
