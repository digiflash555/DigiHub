...
const AttendanceScanner = () => {
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            const recordsRes = await axios.get(`/api/attendance/records/${selectedEvent}`);
            ...
            let settings = {
                ...
                iicLogo: '', digiflashLogo: '',
                associationCoordinatorSign: '', hodSign: ''
            };
            try {
            ...
            const drawHeader = (docInstance) => {
                ...
                docInstance.text('Attendance Sheet', 105, titleY, { align: 'center' });
            ...
            // Call it once on the first page
            drawHeader(doc);
            ...
            if (!isTeamEvent) {
                body = allRecords.map((reg, index) => {
                    ...
                    return [(index + 1).toString(), participant.registrationNumber || '-', participant.username || 'N/A', classDept, ''];
                });
            } else {
                const teamsMap = {};
                const teamOrder = [];
                allRecords.forEach(reg => {
                    ...
                    if (!teamsMap[teamId]) { teamsMap[teamId] = { members: [] }; teamOrder.push(teamId); }
                    teamsMap[teamId].members.push(reg);
                });
                let teamCounter = 0;
                teamOrder.forEach(teamId => {
                    teamCounter++;
                    const members = teamsMap[teamId].members;
                    members.forEach((reg, index) => {
                        ...
                        const classDept = (participant.yearAndDept && sectionVal)
                            ? `${participant.yearAndDept} - ${sectionVal}`
                            : (participant.yearAndDept || participant.department || '-');
                        const rowData = [participant.registrationNumber || '-', participant.username || 'N/A', classDept, ''];
                        if (index === 0) {
                            body.push([{ content: teamCounter.toString(), rowSpan: members.length, styles: { halign: 'center', valign: 'middle' } }, ...rowData]);
                        } else {
                            body.push(rowData);
                        }
                    });
            ...
            // Load signatures into a positional array — unique per participant,
            // not keyed by registrationId (which is shared across a team).
            ...
            await Promise.all(
                sortedRecordsForTable.map(async (reg, i) => {
                    ...
                    if (!sigSource) return;
                    const sigUrl = sigSource.startsWith('data:') ? sigSource : getImageUrl(sigSource);
                    try {
            ...
            autoTable(doc, {
                startY: headerHeight, margin: { top: 15 }, head, body, theme: 'grid',
                        ...
                        if (sigData) {
                            try {
                                ...
                                const fmt = sigData.startsWith('data:image/jpeg') ? 'JPEG'
                                    ...
                                    : 'PNG';
                                doc.addImage(sigData, fmt, data.cell.x + (data.cell.width - imgSize * 2) / 2, data.cell.y + 2, imgSize * 2, imgSize);
                            } catch { /* skip */ }
                        }
                    }
                }
            ...
            let finalY = (doc.lastAutoTable ? doc.lastAutoTable.finalY : headerHeight) + 20;
            if (finalY > 265) { doc.addPage(); finalY = 25; }
