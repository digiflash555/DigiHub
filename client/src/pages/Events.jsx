...
const Events = () => {
    ...
    const filteredEvents = events.filter(event => {
        ...
        if (timeFilter === 'Active') {
            matchesTime = ['Upcoming', 'Ongoing'].includes(event.status);
        } else if (timeFilter === 'Past') {
            matchesTime = ['Completed', 'Cancelled'].includes(event.status);
                ...
                <AnimatePresence mode="popLayout">
                    {isLoading ? (
                        ...
                        filteredEvents.map((event, i) => {
                            const statusColors = {
                                Upcoming: { dot: 'bg-emerald-400', text: 'text-emerald-500' },
                                Ongoing: { dot: 'bg-amber-400', text: 'text-amber-500' },
