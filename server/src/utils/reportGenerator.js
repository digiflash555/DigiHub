...
const generatePDFReport = async (registrations, event, options = {}) => {
    ...
    const drawHeader = (docInstance) => {
        ...
        // Wrap & compute symposium name/type BEFORE rendering them
        docInstance.setFont("helvetica", "bold");
        docInstance.setFontSize(11.5);
        const sympNameLines = docInstance.splitTextToSize(settings.symposiumName || 'DIGIFLASH 2026', 130);

        docInstance.setFontSize(8.5);
        const sympTypeLines = docInstance.splitTextToSize(settings.symposiumType || 'National Level Technical Symposium', 130);
        ...
        const wrapTextWithIndent = (text, indentWidth, maxWidth) => {
            ...
            lines.forEach((line, idx) => {
                ...
                if (docInstance.getTextWidth(line) <= allowedWidth) {
                    finalLines.push(line);
                } else {
                    let tempLine = '';
                    for (let char of line) {
                        if (docInstance.getTextWidth(tempLine + char) <= allowedWidth) {
                            tempLine += char;
                        } else {
        ...
        let titleLines = wrapTextWithIndent(event.title, nameLabelWidth, maxCellWidth);
        ...
        const boxTop = Math.max(42, currentHeaderY + (sympTypeLines.length * 4.5) + 4);
...
const generateFeedbackPDFReport = async (feedbacks, event, options = {}) => {
    ...
    const drawHeader = (docInstance) => {
        // Returns the computed headerHeight so the outer code can position content correctly
        // Left Logo (IIC)
        if (iicLogoObj) {
            try {
                docInstance.addImage(iicLogoObj.data, iicLogoObj.format, 15, 12, 24, 24);
        ...
        docInstance.setFont("helvetica", "bold");
        docInstance.setFontSize(11.5);
        let sympNameLines = docInstance.splitTextToSize(settings.symposiumName || 'DIGIFLASH 2026', 130);

        docInstance.setFontSize(8.5);
        let sympTypeLines = docInstance.splitTextToSize(settings.symposiumType || 'National Level Technical Symposium', 130);
        ...
        const titleY = boxBottomY + 8;
        const statsY = titleY + 8;
        const headerHeight = statsY + 6.5;
        ...
        const totalParticipants = options.totalParticipants || totalResponses;
        const percentage = totalParticipants > 0 ? Math.round((totalResponses / totalParticipants) * 100) : 0;
        
        docInstance.setFont("helvetica", "normal");
        docInstance.setFontSize(9);
        docInstance.setTextColor(71, 85, 105);
        const statsText = `Total Participants: ${totalParticipants}   |   Total Responses: ${totalResponses} (${percentage}%)`;
        docInstance.text(statsText, 105, statsY, { align: 'center' });
        return headerHeight;
    };
    ...
    event.feedbackForm.forEach(field => {
        ...
        if (['dropdown', 'radio', 'checkbox'].includes(field.type)) {
            ...
            const textAnswers = answers.map(a => ({ text: String(a) }));
            summary[field.label] = { type: 'text', answers: textAnswers };
        }
    });

    // ── Draw Page 1: Analytics ───────────────────────────────────
    // Draw header once on the first page; it returns the computed headerHeight
    const headerHeight = drawHeader(doc);
    ...
    // Add Response Stats on the right
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    doc.setTextColor(71, 85, 105);
    const totalParticipants = options.totalParticipants || 0;
    const responsesCount = feedbacks.length;
    const responsePercentage = totalParticipants ? ((responsesCount / totalParticipants) * 100).toFixed(1) : 0;
    const statsText = `Total Participants: ${totalParticipants}  |  Responses: ${responsesCount} (${responsePercentage}%)`;
    doc.text(statsText, 190 - doc.getTextWidth(statsText), currentY);
    ...
    for (const [label, info] of Object.entries(summary)) {
        ...
        if (info.type === 'counts') {
            ...
            // Draw a rectangle for text responses with limited height
            const maxTextHeight = 30; // Maximum height for text responses
            const textResponses = info.answers.slice(0, 3);
            
            // Draw rectangle background
            doc.setFillColor(248, 250, 252);
            doc.setDrawColor(203, 213, 225);
            doc.setLineWidth(0.3);
            doc.rect(25, currentY - 2, 155, maxTextHeight, 'FD');
            
            let textY = currentY + 3;
            textResponses.forEach(({ text }, idx) => {
                if (textY > currentY + maxTextHeight - 5) return;
            ...
            if (info.answers.length > 3) {
                ...
                doc.text(`+ ${info.answers.length - 3} more`, 27, textY);
            }
            
            currentY += maxTextHeight + 5;
        }
    ...
    feedbacks.forEach((fb, idx) => {
        ...
        event.feedbackForm.forEach((f, fi) => {
            ...
            // Answer - limit text field inputs to specific rectangle area
            if (f.type === 'textarea' || f.type === 'text') {
                // Draw rectangle for text answers
                const answerBoxHeight = 25;
                const answerBoxWidth = 112;
                const answerBoxX = 76;
                const answerBoxY = currentY + 2;
                
                doc.setFillColor(248, 250, 252);
                doc.setDrawColor(203, 213, 225);
                doc.setLineWidth(0.3);
                doc.rect(answerBoxX, answerBoxY, answerBoxWidth, answerBoxHeight, 'FD');
                
                // Truncate and display text within the box
                doc.setFont("helvetica", "normal");
                doc.setFontSize(7);
                doc.setTextColor(30, 41, 59);
                
                let displayAns = ansStr;
                const maxChars = 120; // Approximate character limit for the box
                if (displayAns.length > maxChars) {
                    displayAns = displayAns.substring(0, maxChars - 3) + '...';
                }
                
                // Wrap text to fit in box
                const wrappedAns = doc.splitTextToSize(displayAns, answerBoxWidth - 6);
                const maxLines = 3;
                const displayLines = wrappedAns.slice(0, maxLines);
                
                displayLines.forEach((line, lineIdx) => {
                    if (lineIdx < maxLines) {
                        doc.text(line, answerBoxX + 3, answerBoxY + 5 + (lineIdx * 6));
                    }
                });
                
                if (wrappedAns.length > maxLines) {
                    doc.setFont("helvetica", "italic");
                    doc.setFontSize(6);
                    doc.setTextColor(156, 163, 175);
                    doc.text('...', answerBoxX + answerBoxWidth - 8, answerBoxY + answerBoxHeight - 4);
                }
                
                currentY += answerBoxHeight + 5;
            } else {
                // For non-text fields, display normally
                doc.setFont("helvetica", "normal");
                doc.setTextColor(30, 41, 59);
