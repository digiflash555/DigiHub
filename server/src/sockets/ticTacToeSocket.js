...
const handleTicTacToeSocket = (io, socket) => {
    ...
    socket.on('create_room', async ({ user }) => {
        if (!user || !user._id) {
        ...
        try {
            ...
            if (settings.gamesEnabled === false) {
                return socket.emit('error', { message: 'Games are currently disabled by the administrator.' });
            }
        } catch (err) {
            console.error('Error fetching settings in tic-tac-toe socket:', err);
        }
