...
const handleLudoOnlineSocket = (io, socket) => {
    ...
    // Create Room
    socket.on('ludo_create_room', async ({ user, playerCount }) => {
        if (!user?._id) return socket.emit('ludo_error', { message: 'Authentication required.' });

        try {
            const settings = await Settings.getSettings();
            if (settings.gamesEnabled === false) {
                return socket.emit('ludo_error', { message: 'Games are currently disabled by the administrator.' });
            }
        } catch (err) {
            console.error('Error fetching settings in ludo socket:', err);
        }
    ...
    // Join Room
    socket.on('ludo_join_room', async ({ roomCode, user }) => {
