...
exports.submitFeedback = async (req, res, next) => {
    try {
        ...
        res.status(201).json({
            ...
            hasCertificate: !!(event.certificateConfig && event.certificateConfig.template)
        });
    } catch (error) {
        next(error);
    }
...
exports.getEventFeedback = async (req, res, next) => {
    try {
        ...
        const feedbackData = feedbacks.map(fb => {
            ...
            if (fb.responses instanceof Map) {
                obj.responses = Object.fromEntries(fb.responses);
            }
            return obj;
        });
...
exports.getFeedbackAnalytics = async (req, res, next) => {
    try {
            ...
            for (const [label, value] of fb.responses.entries()) {
                if (summary[label]) {
                    if (Array.isArray(value)) {
                        ...
                        // For text/number fields, collect all responses
        ...
        // Compute avg/min/max for number-type fields
        event.feedbackForm.forEach(field => {
            if (field.type === 'number' && summary[field.label]) {
                const nums = (summary[field.label].recentResponses || [])
                    .map(Number)
                    .filter(n => !isNaN(n));
                if (nums.length) {
                    summary[field.label].avg = (nums.reduce((a, b) => a + b, 0) / nums.length).toFixed(1);
                    summary[field.label].min = Math.min(...nums);
                    summary[field.label].max = Math.max(...nums);
                }
            }
        });
    ...
    try {
        ...
        feedbacks.forEach(fb => {
            ...
            event.feedbackForm.forEach(f => {
                ...
                row.push(Array.isArray(val) ? val.join(', ') : (val !== undefined ? String(val) : ''));
            });
...
exports.exportFeedbackPDF = async (req, res, next) => {
    try {
        const authorized = await isAuthorizedForFeedback(req.user, req.params.eventId, ['Admin', 'Association Member', 'Faculty', 'Class Coordinator', 'Program Coordinator']);
        if (!authorized) {
            res.status(403);
            throw new Error('You are not authorized to export feedback for this event.');
        }
        ...
        const totalParticipants = await Registration.countDocuments({ event: req.params.eventId, attendanceStatus: true });
