...
exports.registerForEvent = async (req, res, next) => {
    try {
        ...
        if (event.participationType === 'Team') {
            ...
            // Trigger automatic email for each team member
            for (let reg of registrations) {
                const p = await User.findById(reg.participant);
                if (p) {
                    emailService.triggerAutomaticEmail('EVENT_REGISTRATION', {
                        participant_name: p.username,
                        event_name: event.title,
                        event_date: event.eventDate ? new Date(event.eventDate).toLocaleDateString() : '',
                        event_time: event.time || '',
                        event_venue: event.venue || '',
                        registration_id: reg.registrationId,
                        organization_name: 'Event Management Team'
                    }, p.email, event._id, p._id);
                }
            }
