...
exports.updateProfile = async (req, res, next) => {
    try {
        ...
        if (user) {
            ...
            // Admin-only fields: students cannot change these
...
// @desc    Upload profile picture (winners only for Participant/Association Member)
...
exports.uploadProfilePicture = async (req, res, next) => {
    try {
        ...
        const restrictedRoles = ['Participant', 'Student'];
        if (restrictedRoles.includes(user.role)) {
            const Winner = require('../models/Winner');
            const winnerRecord = await Winner.findOne({ participant: user._id });
            if (!winnerRecord) {
                res.status(403);
                throw new Error('Only winners are allowed to upload a profile picture.');
            }
        }
...
exports.searchUsers = async (req, res, next) => {
    try {
        ...
        if (roleStr) {
            ...
            matchStage.$and.push({ role: { $in: roles } });
        }

        const users = await User.find(matchStage).select('username email registrationNumber role').limit(10);
...
exports.getPublicAssociationMembers = async (req, res, next) => {
    try {
        const members = await User.find({
            role: { $in: ['Admin', 'Association Member', 'Student Coordinator', 'Head of the Department'] },
...
exports.getClassStudents = async (req, res, next) => {
    try {
        let filter = {
            role: { $in: ['Participant', 'Student', 'Association Member', 'Student Coordinator'] }
        };
