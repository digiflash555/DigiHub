...
const resolveRecipients = async (groups) => {
    ...
    const classGroups = groups.filter(g => g.startsWith('Class: '));
    if (classGroups.length > 0) {
        classGroups.forEach(cg => {
            const classStr = cg.replace('Class: ', '');
            if (classStr.includes(' - ')) {
                const [yearAndDept, section] = classStr.split(' - ');
                query.push({ role: { $in: ['Participant', 'Student'] }, yearAndDept, section });
            } else {
                query.push({ role: { $in: ['Participant', 'Student'] }, yearAndDept: classStr });
            }
        });
    }
...
exports.getClassGroups = async (req, res) => {
    try {
        const users = await User.find({ role: { $in: ['Participant', 'Student'] } }, 'yearAndDept section email');
        const classMap = {};
        users.forEach(u => {
            if (u.yearAndDept) {
                const key = u.section ? `${u.yearAndDept} - ${u.section}` : u.yearAndDept;
                if (!classMap[key]) classMap[key] = 0;
                classMap[key]++;
            }
        });
        const classGroups = Object.keys(classMap).map(name => ({
            name: `Class: ${name}`,
            count: classMap[name]
