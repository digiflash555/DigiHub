...
exports.exportNominationPDF = async (req, res, next) => {
    try {
        ...
        if (isEnabled('section')) doc.text(`${label('section', 'Section')}: ${cleanSection(nomination.personalInfo?.section || nomination.user.section)}`);
...
exports.exportAllNominationsPDF = async (req, res, next) => {
    try {
        ...
        nominations.forEach((nom, idx) => {
            ...
            if (isEnabled('section')) col(label('section', 'Section'), cleanSection(nom.personalInfo?.section || nom.user?.section));
            doc.moveDown(0.7);
